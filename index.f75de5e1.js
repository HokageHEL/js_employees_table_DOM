...
function sortTable(arr, count, header, index) {
    return arr.sort((rowA, rowB)=>{
        ...
        if (header.textContent === "Age") return parseInt(cellA) - parseInt(cellB);
    ...
    switch(formValidation(newEmployee)){
        ...
        default:
            for(const key in newEmployee){
                ...
                if (key === "salary") {
                    const validSalary = new Intl.NumberFormat("en-US").format(newEmployee[key]);
...
function formValidation(employe) {
    if (employe.name.length < 4) return "name";
    if (employe.position.length < 4) return "position";
    if (employe.age < 18 || employe.age > 90) return "age";
}
